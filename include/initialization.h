...
// Verifica se um indivíduo é viável (respeita todas as capacidades)
...
void repair(Individual *ind)
{
    ...
    ItemRatio items_to_check[NUM_ITENS];
    int items_count = 0;
...
void initPop()
{
    ...
    // (O ideal é mover toda a alocação de memória para uma função separada)
    ...
    // Alocar memória para sub-populações (se você for usá-las)
